...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ns0="http://hl7.org/fhir" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" exclude-result-prefixes="ns0 xs fn">
	...
	<xsl:template match="/">
		<patientDataFact>
			<xsl:attribute name="xsi:noNamespaceSchemaLocation" namespace="http://www.w3.org/2001/XMLSchema-instance" select="'JSONFact_Med.xsd'"/>
			...
			<xsl:for-each select="ns0:PrescriptionList">
				...
				<xsl:for-each select="$var3_resultof_filter">
					...
					<xsl:variable name="var8_value" as="node()?" select="ns0:status/@value"/>
					<xsl:variable name="var9_dosageInstruction" as="node()+" select="$var5_resultof_first/ns0:dosageInstruction"/>
					<xsl:variable name="var10_value" as="node()?" select="ns0:prescribed/@value"/>
					...
					<facts>
						<xsl:if test="fn:exists($var10_value)">
							...
							<xsl:variable name="var13_resultof_cast" as="xs:string" select="fn:string($var10_value)"/>
							<orderDate>
						...
						<xsl:for-each select="($var9_dosageInstruction/ns0:doseQuantity/ns0:units)[fn:exists(@value)]">
							<unit>
								<xsl:sequence select="fn:string(@value)"/>
							</unit>
						</xsl:for-each>
						<xsl:for-each select="($var9_dosageInstruction/ns0:doseQuantity/ns0:value)[fn:exists(@value)]">
							<dose>
								<xsl:sequence select="xs:string(xs:decimal(fn:string(@value)))"/>
							</dose>
						</xsl:for-each>
						<xsl:for-each select="($var9_dosageInstruction/ns0:route/ns0:text)[fn:exists(@value)]">
							<route>
								<xsl:sequence select="fn:string(@value)"/>
							</route>
						</xsl:for-each>
						<frequency>TDB</frequency>
						<xsl:for-each select="($var9_dosageInstruction/ns0:schedule/ns0:extension/ns0:valueString)[fn:exists(@value)]">
							<schedule>
								<xsl:sequence select="fn:string(@value)"/>
							</schedule>
						</xsl:for-each>
						...
						<xsl:if test="fn:exists($var8_value)">
							<status>
								<xsl:sequence select="fn:string($var8_value)"/>
							</status>
						</xsl:if>
